...
baselink = 'https://maktaba.tetea.org/exam-results/FTNA2022/ftna.htm'
masterlist = []
year = str(2022)
...
with sync_playwright() as pw:
    ...
    for ischool in schoollinks:
        ...
        for row in rows:
            ...
            if len(cells) == 0:
                continue
            if not re.match(r'[A-Z]?\d{1,4}\/\d{1,4}', cells[0].inner_text()):
                continue
...
with open('V:/FHSS-JoePriceResearch/RA_work_folders/Winthrop_Jeanfreau/TanzaniaProject/TanzaniaCSVs/TZF2ExamResults2022Raw.csv', 'w') as work_file:
